...
def _tb(slide, text, x, y, w, h, size, bold=False, italic=False,
    ...
    for i, part in enumerate(str(text).split("**")):
        ...
        r.font.size = Pt(size); r.font.bold = bold or (i%2==1)
        r.font.italic = italic; r.font.name = face
        if color: r.font.color.rgb = _c(color)
    return bx

def _bullets(slide, points, x, y, w, h, size=15, icon="▸",
             ic=None, tc=None, face="Calibri", maxp=6):
    if not points: return
    ic = ic or _BASE["s"]; tc = tc or _BASE["td"]
    pts = [_safe_str(p) for p in points[:maxp] if _safe_str(p)]
    if not pts: return
    n = len(pts)
    if   n <= 2: size = min(24, size+5)
    elif n == 3: size = min(22, size+3)
    elif n == 4: size = min(19, size+1)
    bx = slide.shapes.add_textbox(_IN(x),_IN(y),_IN(w),_IN(h))
    tf = bx.text_frame; tf.word_wrap = True
    ...
    for pt in pts:
        ...
        ir.font.size=Pt(size-1); ir.font.bold=True
        ir.font.name=face; ir.font.color.rgb=_c(ic)
        for i,part in enumerate(pt.split("**")):
            if not part: continue
            r=p.add_run(); r.text=part
            r.font.size=Pt(size); r.font.bold=(i%2==1)
...
def _draw_header(slide, title, subtitle, num, theme, profile):
    ...
    # Number badge position (right side)
    nbx, nby, nbw, nbh = W-1.55, 0.22, 0.60, 0.60

    if mode == "sidebar":
        _rect(slide, 0, 0, 0.42, H, theme["p"])
        _rect(slide, 0.42, 0, W-0.42, H, _mix(theme["dk"],theme["p"],0.18))
    elif mode == "band":
    ...
    title_w = nbx - tx - 0.10
    tsize = max(20, 30 - max(0, len(title)-42)//8*4)
    _tb(slide, title, tx, ty, title_w, th, tsize,
        bold=True, color=theme["tl"], face=theme["hf"], shrink=True)
    if subtitle:
        sc = _mix(theme["tl"], theme["s"], 0.35)
        ssize = max(10, 13 - max(0, len(subtitle)-60)//20)
        _tb(slide, subtitle, tx, 0.96, title_w, 0.38, ssize,
            italic=True, color=sc, face=theme["bf"], shrink=True)

    _oval(slide, nbx, nby, nbw, nbh, theme["a"])
    ns = str(num); npt = 13 if len(ns)>1 else 15
    _tb(slide, ns, nbx+0.02, nby+0.06, nbw-0.04, nbh-0.10, npt,
        bold=True, color=theme["tl"], face=theme["hf"], align=PP_ALIGN.CENTER)
...
def _add_logo(slide, logo_path, theme):
    ...
    try:
        bw, bh, bmar_r, bmar_t = 1.65, 0.90, 0.18, 0.18
        bx = SW - bw - bmar_r; by = bmar_t
        pad = 0.15; mh = bh-pad*2; mw = bw-pad*2
        with Image.open(logo_path) as img: iw,ih = img.size
        ratio = iw/ih if ih else 1.0
        lh = min(mh, mw/ratio); lw = lh*ratio
        _rect(slide, bx, by, bw, bh, (255,255,255), line=theme["s"], lw=1.0)
        slide.shapes.add_picture(logo_path,
            _IN(bx+(bw-lw)/2), _IN(by+(bh-lh)/2), _IN(lw), _IN(lh))
    except Exception as e:
        print(f"[logo] {e}")
...
def render_title_cover(slide, spec, num, theme, profile, logo_path):
    ...
    tsize = max(36, 54 - max(0, len(title)-30)//6*4)
    ...
    if subtitle:
        ssize = max(14, 18 - max(0, len(subtitle)-60)//20)
    ...
    if points:
        ...
        for i, pt in enumerate(points):
            ...
            _tb(slide, pt, cx+0.12, 5.16, card_w-0.24, 0.76, 15,
                bold=True, color=hc if mode=="dark_full" else theme["td"],
...
def render_section_index(slide, spec, num, theme, profile, logo_path):
    ...
    if not sections:
        ...
        _add_logo(slide, logo_path, theme); return
    ...
    for i, sec in enumerate(sections):
        ...
        cy = iy + row*(ch+gap)
        ac = accs[i % len(accs)]
        _rect(slide, cx, cy, cw, ch, _mix(theme["card"],ac,0.90), line=ac, lw=1.0)
        _rect(slide, cx, cy, 0.72, ch, ac)
        _tb(slide, f"{i+1:02d}", cx+0.08, cy+0.14, 0.56, ch-0.24, 18,
            bold=True, color=theme["tl"], face=theme["hf"], align=PP_ALIGN.CENTER)
        _tb(slide, sec, cx+0.86, cy+0.16, cw-1.00, ch-0.28,
            18 if cols==1 else 15, bold=True, color=theme["td"],
            face=theme["bf"], shrink=True)
    _add_logo(slide, logo_path, theme)
...
def render_two_column(slide, spec, num, theme, profile, logo_path):
    ...
    _tb(slide, lt, ix+0.10, iy+0.08, cw-0.20, hh-0.12, 15,
        bold=True, color=theme["tl"], face=theme["hf"])
...
def render_big_stat(slide, spec, num, theme, profile, logo_path):
    ...
    _tb(slide, label, ix+0.10, iy+2.25, pw-0.20, 0.75, 15,
        color=theme["s"], face=theme["bf"], align=PP_ALIGN.CENTER)
    if source:
        _tb(slide, source, ix+0.10, iy+3.05, pw-0.20, 0.50, 11,
            italic=True, color=(180,210,255), face=theme["bf"], align=PP_ALIGN.CENTER)
...
def render_timeline(slide, spec, num, theme, profile, logo_path):
    ...
    for i, step in enumerate(steps):
        ...
        _tb(slide, step["label"], cx_c+0.10, cy+0.08, cw_-0.20, 0.42, 13,
            bold=True, color=ac, face=theme["hf"])
        dh = ch-0.56
        if dh > 0.12:
            _tb(slide, step["detail"], cx_c+0.10, cy+0.52, cw_-0.20, dh, 12,
                color=theme["td"], face=theme["bf"])
    _add_logo(slide, logo_path, theme)
...
def render_icon_grid(slide, spec, num, theme, profile, logo_path):
    ...
    for i, gi in enumerate(items):
        ...
        raw = _safe_str(gi.get("icon", gi.get("title","A")))
        ch1 = next((c.upper() for c in raw if c.isascii() and c.isalnum()), "A")
        _tb(slide, ch1, iox+0.04, ioy+0.08, IR*2-0.08, IR*1.6, 17,
            bold=True, color=(255,255,255), face=theme["hf"], align=PP_ALIGN.CENTER)
        tx = iox+IR*2+0.16; tw = cw_-(iox-cx)-IR*2-0.22
        _tb(slide, _safe_str(gi.get("title","")), tx, cy+0.12, tw, 0.46, 16,
            bold=True, color=theme["p"], face=theme["hf"])
...
def render_case_study(slide, spec, num, theme, profile, logo_path):
    ...
    if result:
        ...
        _tb(slide, f"  {result}", ix+0.14, cur_y+0.10, iw-0.28, rh-0.16, 13,
            bold=True, color=theme["tl"], face=theme["bf"])
        cur_y += rh+0.06
    ...
    for i, m in enumerate(metrics[:3]):
        ...
        _tb(slide, lbl, ix+0.14, my+0.04, mlw, 0.30, 13, color=theme["td"])
        _rect(slide, bar_x, my+0.06, bar_w, 0.22, (225,232,245))
        _rect(slide, bar_x, my+0.06, bar_w*(val/100), 0.22, theme["a2"])
        _tb(slide, f"+{val}%", bar_x+bar_w*(val/100)+0.06, my, 0.55, 0.30, 12,
            bold=True, color=theme["p"])
        metrics_used = (i+1)*mh
...
def render_table(slide, spec, num, theme, profile, logo_path):
    ...
    for ci, col in enumerate(cols):
        ...
        _tb(slide, col, cx+pad, iy+pad, col_w-pad*2, hh-pad*2, 13,
            bold=True, color=theme["tl"], face=theme["hf"], align=PP_ALIGN.CENTER, shrink=True)
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, cell in enumerate(cells):
            ...
            _tb(slide, cell, cx+pad, ry+pad, col_w-pad*2, row_h-pad, 12,
                color=theme["td"], face=theme["bf"], shrink=True)
    _add_logo(slide, logo_path, theme)
...
def render_chart(slide, spec, num, theme, profile, logo_path):
    ...
    for i, item in enumerate(chart_data):
        ...
        _tb(slide, str(val), bar_x+bar_w+0.06, y+0.01, 0.35, row_h-0.02, 12,
            bold=True, color=theme["p"], align=PP_ALIGN.RIGHT)
...
def render_image_text_split(slide, spec, num, theme, profile, logo_path):
    ...
    if cap:
        _tb(slide, cap, imx+0.14, iy+ih-0.48, img_w-0.28, 0.38, 13,
            bold=True, color=theme["td"], face=theme["bf"], align=PP_ALIGN.CENTER)
...
def render_hybrid_insight(slide, spec, num, theme, profile, logo_path):
    ...
    _rect(slide, ix, iy, lw, ih, _mix(theme["card"],theme["s"],0.86), line=theme["s"], lw=1.0)
    sfont = 52 if len(stat)<=4 else 42
    _tb(slide, stat, ix+0.12, iy+0.30, lw-0.24, 1.15, sfont,
        bold=True, color=theme["p"], face=theme["hf"], align=PP_ALIGN.CENTER)
    _tb(slide, label, ix+0.12, iy+1.45, lw-0.24, 0.42, 13,
        color=theme["td"], face=theme["bf"], align=PP_ALIGN.CENTER)
    ...
    if chart:
        ...
        for i, row in enumerate(chart):
            ...
            _tb(slide, str(v), bar_x+bar_w+0.03, y-0.01, 0.26, row_h, 10,
                bold=True, color=theme["p"], align=PP_ALIGN.RIGHT)
...
def _repair_slide(slide: dict, topic: str) -> dict:
    layout = _canon(slide.get("layout","bullets"))
    template = _REPAIR_PROMPT.get(layout)
    if not template: return slide
...
def _normalize_slide(slide: dict, idx: int, topic: str) -> dict:
    ...
    if layout == "two_column":
        ...
        for g in items:
            ...
            t = _safe_str(g.get("title","")); d = _safe_str(g.get("detail",""))
            if t: cleaned.append({"icon":_safe_str(g.get("icon","▸")),"title":t,"detail":d})
        slide["grid_items"] = cleaned[:4]
...
"case_study":
  ...
  content: ["3-5 supporting bullet strings"]
...
def create_ppt(slide_data, topic: str,
    ...
    for i, spec in enumerate(slides, start=1):
        ...
        if not isinstance(spec.get("style"), dict): spec["style"] = {}
